...
export default function Register() {
      ...
      members: [
        {
          ...
          organization: "ANITS" as Organization,
          customOrganization: "",
          year: "",
          department: "",
          accommodation: false,
  ...
  const validateStep = (step: number) => {
    ...
    switch (step) {
      case 1:
        ...
        if (!teamLead.fullName) newErrors.teamLeadName = "Team lead name is required";
        if (!teamLead.email) newErrors.teamLeadEmail = "Team lead email is required";
        if (!teamLead.whatsapp) newErrors.teamLeadPhone = "Team lead phone is required";
        if (!teamLead.role) newErrors.teamLeadRole = "Role is required";
        if (!teamLead.organization) newErrors.teamLeadOrg = "Organization is required";
        if (teamLead.organization === "Other" && !teamLead.customOrganization) {
          newErrors.teamLeadCustomOrg = "Please specify organization";
        }
        if ((teamLead.role === "Student" || teamLead.role === "Academician") && !teamLead.year) {
          newErrors.teamLeadYear = "Year is required";
        }
        if ((teamLead.role === "Student" || teamLead.role === "Academician") && !teamLead.department) {
          newErrors.teamLeadDept = "Department is required";
        }
        break;
      ...
      case 3:
        formData.members.slice(1).forEach((member, index) => {
          ...
          if (!member.fullName) newErrors[`member${memberIndex}Name`] = "Name is required";
          if (!member.email) newErrors[`member${memberIndex}Email`] = "Email is required";
          if (!member.whatsapp) newErrors[`member${memberIndex}Phone`] = "WhatsApp number is required";
          
          if (!member.sameAsLead) {
            if (!member.role) newErrors[`member${memberIndex}Role`] = "Role is required";
            if (!member.organization) newErrors[`member${memberIndex}Org`] = "Organization is required";
            if (member.organization === "Other" && !member.customOrganization) {
              newErrors[`member${memberIndex}CustomOrg`] = "Please specify organization";
            }
            if ((member.role === "Student" || member.role === "Academician") && !member.year) {
              newErrors[`member${memberIndex}Year`] = "Year is required";
            }
            if ((member.role === "Student" || member.role === "Academician") && !member.department) {
              newErrors[`member${memberIndex}Dept`] = "Department is required";
            }
          }

          // Validate phone number (10 digits)
          if (member.whatsapp && !/^\d{10}$/.test(member.whatsapp)) {
            newErrors[`member${memberIndex}Phone`] = "Enter valid 10-digit phone number";
          }

          // Validate email
          if (member.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(member.email)) {
            newErrors[`member${memberIndex}Email`] = "Enter valid email address";
          }
        });
        break;
  ...
  const clearCopyFromLead = (index: number) => {
    const newMembers = [...formData.members];
    newMembers[index] = { ...newMembers[index], sameAsLead: false };
    setFormData({ ...formData, members: newMembers });
  };
