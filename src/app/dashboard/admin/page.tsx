...
import { Gallery, GalleryResponse } from '@/entities/Gallery';
...
export default function AdminDashboard() {
  ...
  const [galleryItems, setGalleryItems] = useState<GalleryResponse[]>([]);
  ...
  const loadAllData = async () => {
    try {
      const [usersData, registrationsData, ticketsData, galleryData] = await Promise.all([
        User.getAll(),
        Registration.getAll(),
        HelpTicket.getAll(),
        Gallery.getAll()
  ...
  const handleGalleryModeration = async (itemId: string, action: 'approve' | 'reject') => {
    try {
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            <Card className="bg-gray-800/40 border-gray-700">
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-gray-400 text-sm">Help Tickets</p>
          ...
          <div className="grid lg:grid-cols-4 gap-6">
                  ...
                  <div className="space-y-4">
                    {qrCode ? (
                      ...
                      <div className="w-32 h-32 bg-white rounded-lg flex items-center justify-center mx-auto">
                        <QrCode className="w-24 h-24 text-red-500/50" />
                      </div>
                    )}
                    <div className="text-center">
                      <Avatar className="w-16 h-16 mx-auto mb-2">
                        <AvatarFallback className="bg-gradient-to-r from-red-500 to-pink-500 text-xl font-bold text-white">
            ...
            <div className="lg:col-span-3">
              <Tabs defaultValue="users" className="w-full">
                <TabsList className="grid w-full grid-cols-5 bg-gray-800/40 mb-8 h-16">
                  ...
                  <TabsTrigger value="games" className="text-gray-400 data-[state=active]:bg-gradient-to-r data-[state=active]:from-purple-500 data-[state=active]:to-pink-500 data-[state=active]:text-white h-full">
                    <div className="text-center">
                      <div className="font-semibold text-sm">Games</div>
                  ...
                  <TabsTrigger value="tickets" className="text-gray-400 data-[state=active]:bg-gradient-to-r data-[state=active]:from-orange-500 data-[state=active]:to-red-500 data-[state=active]:text-white h-full">
                    <div className="text-center">
                      <div className="font-semibold text-sm">Tickets</div>
                  ...
                  <TabsTrigger value="gallery" className="text-gray-400 data-[state=active]:bg-gradient-to-r data-[state=active]:from-pink-500 data-[state=active]:to-rose-500 data-[state=active]:text-white h-full">
                    <div className="text-center">
                      ...
                      <div className="text-xs opacity-70">Moderation</div>
                    </div>
                  </TabsTrigger>
                </TabsList>
                          ...
                          <div key={index} className="p-4 bg-gray-700/30 rounded-lg">
                            <div className="flex justify-between items-center">
                              <div className="flex items-center space-x-3">
                                ...
                                <div>
                                  <h3 className="text-white font-medium">{userData.full_name}</h3>
                ...
                <TabsContent value="games" className="space-y-6">
                  <div className="grid md:grid-cols-2 gap-6">
                    <Card className="bg-gray-800/40 backdrop-blur-sm border-gray-700">
                      <CardHeader>
                        <CardTitle className="text-white">QR Quest Leaderboard</CardTitle>
                      </CardHeader>
                      <CardContent>
                      ...
                      <CardContent>
                        <div className="space-y-4">
                          <div className="flex justify-between">
                            <span className="text-gray-400">Total QR Scans:</span>
                            <span className="text-white">{gameStats.totalScans || 0}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-400">Active Players:</span>
                            <span className="text-white">{gameStats.activePlayers || 0}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-400">Imposter Suspects:</span>
                            <span className="text-white">{gameStats.totalSuspects || 0}</span>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
                ...
                <TabsContent value="gallery" className="space-y-6">
                  <Card className="bg-gray-800/40 backdrop-blur-sm border-gray-700">
                    <CardHeader>
                      <CardTitle className="text-white">Gallery Moderation</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {galleryItems.map((item, index) => (
                          <div key={index} className="bg-gray-700/30 rounded-lg p-3">
                            ...
                            <p className="text-gray-400 text-xs">by {item.uploaded_by}</p>
                            <div className="flex space-x-2 mt-2">
                              <Button
                                size="sm"
                                onClick={() => handleGalleryModeration(item.id, 'approve')}
                              ...
                              <Button
                                size="sm"
                                onClick={() => handleGalleryModeration(item.id, 'reject')}
                                variant="outline"
                                className="flex-1"
                              >
                                Reject
